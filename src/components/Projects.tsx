    ...
    <div className='flex flex-col items-center w-full py-10 md:py-20'>
      ...
      >
        <h2 className='text-4xl md:text-8xl font-black text-cyan-400 uppercase tracking-tighter transition-all duration-300 drop-shadow-[0_0_15px_rgba(34,211,238,0.8)] group-hover:drop-shadow-[0_0_40px_rgba(34,211,238,1)]'>
          Misiones <span className='italic font-light opacity-80 text-3xl md:text-7xl'>Ejecutadas</span>
        </h2>
      </motion.div>
      ...
      <div className="relative w-full h-80 md:h-137.5 flex items-center justify-center overflow-hidden md:overflow-visible">
        <AnimatePresence mode='popLayout'>
          {PROJECTS.map((project, i) => {
            const position = i === index ? 0 : i === (index + 1) % PROJECTS.length ? 1 : -1;
            ...
            return (
              ...
              >
                ...
                {position === 0 && (
                  ...
                  >
                    <h3 className="text-lg md:text-4xl font-bold text-white mb-2 md:mb-4">{project.title}</h3>
                    <div className="flex gap-3 md:gap-4">
                      <a href={project.github} target="_blank" rel="noopener noreferrer" className="p-2 md:p-3 bg-white/10 hover:bg-cyan-500 rounded-full transition-colors">
                        <FiGithub className="text-lg md:text-2xl" />
                      </a>
                      <a href={project.link} target="_blank" rel="noopener noreferrer" className="p-2 md:p-3 bg-white/10 hover:bg-cyan-500 rounded-full transition-colors">
